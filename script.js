...
document.getElementById("fileInput").addEventListener("change", function (e) {
    ...
    reader.onload = function (evt) {
        let data;
        try {
            if (file.name.endsWith(".json")) data = JSON.parse(evt.target.result);
            else data = csvToArray(evt.target.result);
        } catch {
...
function calculateHMPI(row) {
    let sum = 0;
    ["Lead","Cadmium","Arsenic"].forEach(m => {
...
function processData(data){
    hmpiData = data.map(r => {
        ...
        return {...r, HMPI:hmpi, Category:cat.text, Color:cat.color};
    });
    populateTable();
    populateCharts();
    populateDashboard();
...
// Charts
let hmpiChart, hmpiLineChart, metalChart, categoryChart, heatmapChart, gaugeChart;
function populateCharts(){
    ...
    hmpiChart = new Chart(document.getElementById("hmpiChart"),{
        type:'bar',
        data:{labels:locations, datasets:[{label:"HMPI", data:hmpiValues, backgroundColor:hmpiData.map(d=>d.Color)}]},
        options:{responsive:true, maintainAspectRatio:false, plugins:{legend:{display:false}}}
    });

    // HMPI Line Chart
    if(hmpiLineChart) hmpiLineChart.destroy();
    hmpiLineChart = new Chart(document.getElementById("hmpiLineChart"),{
        type:'line',
        data:{labels:locations,datasets:[{label:"HMPI Trend",data:hmpiValues,fill:false,borderColor:'#0288d1',tension:0.3,pointBackgroundColor:'#0288d1'}]},
...
function populateDashboard(){
    ...
    gaugeChart = new Chart(document.getElementById("gaugeChart"),{
        ...
        options:{responsive:true, maintainAspectRatio:false,circumference:Math.PI,rotation:-Math.PI,cutout:'70%',plugins:{legend:{display:false},tooltip:{enabled:false}}}
    });
}
...
document.getElementById("exportBtn").addEventListener("click",()=>{
    ...
    hmpiData.forEach(r=>csv+=`${r.Location},${r.Latitude},${r.Longitude},${r.Lead},${r.Cadmium},${r.Arsenic},${r.HMPI},${r.Category}\n`);
